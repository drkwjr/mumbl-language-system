...
**Audio (native-recorded — the gold tier)** — 124 LearnAkan studio tracks (**447 min**) paired to their text: `ingest/audio_pairing.py` segments each guide by its `track N` markers (1:1 with `Track N.mp3`) → a track-level manifest (`_restricted/audio-corpus.jsonl`) of **2,544 phrases** with audio path + duration. `ingest/audio_verify.py` ASR-checks that each track speaks its paired text (verify-not-trust: 62% mean Twi-word overlap on a sample, a floor — long tracks ASR-truncate; no track near 0% = pairing holds). This is the foundation for native-pronunciation playback, Twi TTS training data (the answer to "Azure has no Twi voice"), and word-level forced alignment (next).
