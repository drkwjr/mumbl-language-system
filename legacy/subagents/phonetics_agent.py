...
class PhoneticsAgent:
    ...
    def detect_dialect_features(self, ipa_string, language="en"):
        ...
        """
        results = {"likely_dialects": [], "features": {}}
        ...
        try:
            ...
            for p in pronunciations:
                ...
                for cluster in clusters:
                    analysis["consonant_clusters"][cluster] = (
                        analysis["consonant_clusters"].get(cluster, 0) + 1
                    )

                # Identify syllable patterns (very simplified)
                if len(word) > 0:
                    syllable_count = max(
                        1, len(re.findall(r"[aeiouæɑɛɪɔʊʌəɒ]+", ipa, re.IGNORECASE))
                    )
                    key = f"{syllable_count}_syllable"
                    analysis["syllable_patterns"][key] = (
                        analysis["syllable_patterns"].get(key, 0) + 1
                    )
            ...
            analysis["consonant_clusters"] = dict(
                sorted(analysis["consonant_clusters"].items(), key=lambda x: x[1], reverse=True)[
                    :20
                ]
            )
