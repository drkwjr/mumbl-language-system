...
class MetadataAgent:
    ...
    def determine_source_type(self, source_name, source_url=None):
        ...
        if any(
            term in source_name_lower
            for term in ["journal", "university", "research", "proceedings", "academia"]
        ):
            return "academic"
        ...
        if any(
            term in source_name_lower
            for term in ["linguistics", "corpus", "grammar", "phonology", "syntax"]
        ):
            return "linguistic_resource"

        # Check for user-contributed content
        if any(
            term in source_name_lower for term in ["forum", "wiki", "blog", "community", "user"]
        ):
        ...
        if source_url:
            ...
            if any(
                term in domain
                for term in ["dictionary", "lexicon", "oxford", "cambridge", "merriam-webster"]
            ):
                return "dictionary"
    ...
    def link_entity_to_source(
        self, entity_type, entity_id, source_id, confidence_score=None, notes=None
    ):
        """
    ...
    def verify_source_url(self, source_id):
        ...
        try:
            ...
            try:
                headers = {"User-Agent": "Mumbl Language Processing System (Verification Bot)"}
                ...
                verification_result = {"verified": False, "url": url, "error": str(e)}
        ...
        try:
            ...
            for source in sources:
                ...
                if abs(old_score - new_score) > 0.01:
                    ...
                    updated_count += 1
                    logger.info(
                        f"Updated reliability score for {source_name}: {old_score} -> {new_score}"
                    )
    ...
    def run_maintenance(self):
        ...
        """
        results = {"sources_reassessed": 0, "sources_verified": 0, "success": False}
