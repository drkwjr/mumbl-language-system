...
class GrammarAgent:
    ...
    def extract_grammar_rule_formula(self, rule_description):
        ...
        formula_match = re.search(
            r"([A-Za-z0-9\+]+)\s*(?:→|->|=)\s*([A-Za-z0-9\+\s]+)", rule_description
        )
        if formula_match:
        ...
        # Look for "X becomes Y" patterns
            ...
            r"([A-Za-z0-9\+]+)\s+becomes\s+([A-Za-z0-9\+\s]+)", rule_description, re.IGNORECASE
        ):
            match = re.search(
                r"([A-Za-z0-9\+]+)\s+becomes\s+([A-Za-z0-9\+\s]+)", rule_description, re.IGNORECASE
            )
    ...
    def analyze_sentence_complexity(self, sentence):
        ...
        score_components = [
            ...
            min((features["conjunction_count"] + features["subordinate_clause_markers"]) / 5, 1)
            * 2,  # Up to 2 points for conjunctions/subordinators
        ]

        complexity_score = round(sum(score_components), 2)

        return {"complexity_score": min(complexity_score, 10), "features": features}
    ...
    def identify_grammar_tone(self, sentence):
        ...
        if (
            pos_tags
            and pos_tags[0][1].startswith("VB")
            and not tokens[0] in ["is", "are", "was", "were", "am", "be", "being", "been"]
        ):
            return "imperative"

        # Check for interrogative (question) tone
        if sentence.endswith("?") or tokens[0] in [
            "what",
            "who",
            "whom",
            "whose",
    ...
    def extract_grammar_rules_from_sentence(self, sentence, language_code="en"):
        ...
        for i, (word, tag) in enumerate(pos_tags):
            ...
            # Verb identification
            elif tag.startswith("VB") and (
                current_phrase["type"] == "SUBJ" or not current_phrase["type"]
            ):
            ...
            # Adverb modifying verb pattern
                ...
                tag.startswith("RB")
                and i < len(pos_tags) - 1
                and pos_tags[i + 1][1].startswith("VB")
            ):
                rules.append(
                    {
            ...
            # Preposition followed by noun phrase
                ...
                tag == "IN"
                and i < len(pos_tags) - 2
                and pos_tags[i + 1][1] in ["DT", "PRP$"]
                and pos_tags[i + 2][1].startswith("NN")
            ):
                rules.append(
                    {
            ...
            for sentence in sentences:
                ...
                for rule in rules:
                    ...
                    if not existing_rule:
                        ...
                        logger.info(
                            f"Created new grammar rule: {rule['rule_name']} (ID: {rule_id})"
                        )
    ...
    def run_batch_processing(self):
        ...
        """
        results = {"sentences_processed": 0, "rules_categorized": 0, "success": False}
