...
try:
    ...
    raise ImportError(
        f"Could not import curator. Install with: pip install -e apps/curator"
    ) from e
...
def score_and_dedupe(man: BatchManifest) -> BatchManifest:
    ...
    with get_connection() as conn:
        ...
        from mumbl_data_contracts.segments import TextSegment, AudioSegment, Labels, SourceRef
        text_segments = []
        ...
        man.metrics["eligible_segments"] = (
            result['stats']['text_segments_eligible'] +
            result['stats']['audio_segments_eligible']
        )
...
def snapshot_and_register(man: BatchManifest) -> BatchManifest:
    ...
    with get_connection() as conn:
        ...
        man.outputs["curated_manifest"] = snapshot.get('snapshot_path', '').replace('.json', '.jsonl')
