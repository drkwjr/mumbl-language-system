...
class TextLID:
    ...
    def __init__(self, model_path: Optional[str] = None):
        ...
        if not FASTTEXT_AVAILABLE:
            raise ImportError("fastText is not installed. Install with: pip install fasttext")
        ...
        try:
            ...
            logger.error("Failed to load fastText model", model_path=self.model_path, error=str(e))
            # Model will download automatically on first use
            # If it fails, user needs to download manually
            raise

    def predict_language(self, text: str, top_k: int = 3) -> List[Tuple[str, float]]:
        """
        ...
        try:
            ...
            logger.error("Text LID prediction failed", text_length=len(text), error=str(e))
            return []
...
def create_text_lid(model_path: Optional[str] = None) -> Optional[TextLID]:
    ...
    try:
        ...
        logger.warning(
            "Text LID model creation failed", error=str(e), note="Text LID will be disabled"
        )
        return None
