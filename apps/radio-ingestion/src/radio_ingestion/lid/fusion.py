...
class LIDFusion:
    ...
    def __init__(
        self, audio_weight: float = 0.5, text_weight: float = 0.4, context_weight: float = 0.1
    ):
        """
    ...
    def normalize_probabilities(self, predictions: List[Tuple[str, float]]) -> Dict[str, float]:
        """
        ...
        # Build dictionary
        result = {lang_code: float(prob) for (lang_code, _), prob in zip(predictions, probs)}
    ...
    def get_primary_language(self, fused_probs: Dict[str, float]) -> Tuple[Optional[str], float]:
        """
...
def create_fusion(
    audio_weight: float = 0.5, text_weight: float = 0.4, context_weight: float = 0.1
) -> LIDFusion:
    """
    ...
    """
    return LIDFusion(
        audio_weight=audio_weight, text_weight=text_weight, context_weight=context_weight
    )
