...
from radio_ingestion.storage.s3_uploader import S3Uploader, LocalStaging, create_uploader
...
class StorageManager:
    ...
    def process_shard(
        ...
        try:
            ...
            if self.s3_uploader and self.s3_uploader.enabled:
                ...
                if s3_url:
                    ...
                    if self.db_conn:
                        ...
                        logger.info(
                            "Shard uploaded to S3, DB updated",
                            shard_id=shard_id,
                            s3_url=s3_url
                        )
            ...
            result["error"] = "No storage backend available (S3 disabled and local staging not configured)"
