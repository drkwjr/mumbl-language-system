...
class FileCleanup:
    ...
    def __init__(self, cleanup_enabled: bool = True, retention_days: int = 7):
        """
    ...
    def cleanup_file(
        self, file_path: str, require_s3_url: bool = True, s3_url: Optional[str] = None
    ) -> bool:
        """
        ...
        if require_s3_url and not s3_url:
            logger.debug(
                "Skipping cleanup (no S3 URL)", file_path=file_path, require_s3_url=require_s3_url
            )
            return False
        ...
        try:
            ...
            logger.info("File cleaned up", file_path=file_path, file_size=file_size, s3_url=s3_url)
    ...
    def cleanup_old_files(
        ...
        try:
            for file_path in dir_path.rglob("*"):
                ...
                if file_mtime < cutoff_time:
                    try:
                        ...
                        logger.warning(
                            "Failed to delete old file", file_path=str(file_path), error=str(e)
                        )
            ...
            logger.error("Cleanup directory scan failed", directory=directory, error=str(e))
            return []
    ...
    def cleanup_shard_files(
        ...
        for shard_path, s3_url in zip(shard_paths, s3_urls):
            success = self.cleanup_file(shard_path, require_s3_url=require_s3, s3_url=s3_url)
            results[shard_path] = success

        deleted_count = sum(1 for success in results.values() if success)

        if deleted_count > 0:
            logger.info("Batch cleanup completed", total=len(shard_paths), deleted=deleted_count)

        return results


def create_cleanup(cleanup_enabled: bool = True, retention_days: int = 7) -> FileCleanup:
