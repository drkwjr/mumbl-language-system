...
class RadioIngestionConfig(BaseModel):
    ...
    radio_browser_api: str = Field(
        default="https://de1.api.radio-browser.info/json", description="Radio Browser API endpoint"
    )

    # Storage paths
    capture_dir: str = Field(
        default="data/radio_shards", description="Local directory for captured audio shards"
    )
    ...
    capture_countries: List[str] = Field(
        default_factory=lambda: ["GHA", "SOM"], description="Country codes to capture"
    )
    ...
    vad_aggressiveness: int = Field(
        default=2, ge=0, le=3, description="WebRTC VAD aggressiveness 0-3"
    )
    ...
    station_refresh_interval_hours: int = Field(
        default=24, description="Hours between station refresh"
    )
    capture_interval_minutes: int = Field(
        default=60, description="Minutes between captures per station"
    )
