...
class WindowExtractor:
    ...
    def __init__(
        ...
        self.vad = VADProcessor(aggressiveness=vad_aggressiveness, sample_rate=self.vad_rate)

        # Initialize music classifier
        self.music_classifier = MusicClassifier(sample_rate=sample_rate, threshold=music_threshold)
    ...
    def extract_speech_windows(self, audio_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            speech_regions = self.vad.filter_by_min_duration(
                speech_regions, self.min_speech_duration
            )
            ...
            speech_segments = [
                (start, end, music_prob)
                for start, end, music_prob in regions_with_music
            ...
            logger.error("Failed to extract speech windows", audio_path=audio_path, error=str(e))
            return []

    def compute_mfcc_features(
        self, audio_data: np.ndarray, n_mfcc: int = 13, n_fft: int = 2048, hop_length: int = 512
    ) -> Optional[np.ndarray]:
        """
        ...
        try:
            mfccs = librosa.feature.mfcc(
                y=audio_data, sr=self.sample_rate, n_mfcc=n_mfcc, n_fft=n_fft, hop_length=hop_length
            )
            ...
            logger.warning("MFCC computation failed", error=str(e))
            return None
    ...
    def process_shard(self, shard_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning("Failed to compute total duration", shard_path=shard_path, error=str(e))
