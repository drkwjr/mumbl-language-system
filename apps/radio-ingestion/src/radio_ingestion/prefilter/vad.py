...
class VADProcessor:
    ...
    def __init__(self, aggressiveness: int = 2, sample_rate: int = 16000, merge_collar_ms: int = 200):
        """
        ...
        if not WEBRTC_VAD_AVAILABLE:
            raise ImportError(
                "webrtcvad is not installed. Install with: pip install webrtcvad"
            )
    ...
    def filter_by_min_duration(self, regions: List[Tuple[float, float]], min_duration: float = 0.5) -> List[Tuple[float, float]]:
        """
