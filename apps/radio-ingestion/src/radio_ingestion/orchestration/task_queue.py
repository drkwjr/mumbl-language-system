    ...
    def transition_to(self, new_state: TaskState, error: Optional[str] = None):
        ...
        if self.on_state_change:
            try:
                ...
                logger.error("State change callback failed", shard_id=self.shard_id, error=str(e))
...
class TaskQueue:
    ...
    async def enqueue(self, task: ShardTask) -> bool:
        ...
        if task.shard_id in self.tasks:
            logger.warning("Task already in queue", shard_id=task.shard_id, state=task.state.value)
            return False
    ...
    async def dequeue(self, timeout: Optional[float] = None) -> Optional[ShardTask]:
        ...
        try:
            ...
            logger.debug("Task dequeued", shard_id=task.shard_id, state=task.state.value)
    ...
    def update_task_state(
        self, shard_id: int, new_state: TaskState, error: Optional[str] = None
    ) -> bool:
        """Update task state"""
    ...
    def get_queue_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "by_state": {state.value: len(self.get_tasks_by_state(state)) for state in TaskState},
        }
    ...
    def clear_completed(self, max_age_hours: int = 24):
        ...
        if to_remove:
            logger.info("Cleared old tasks", count=len(to_remove), max_age_hours=max_age_hours)
...
class PipelineProcessor:
    ...
    async def _run_callback(self, callback: Callable, task: ShardTask):
