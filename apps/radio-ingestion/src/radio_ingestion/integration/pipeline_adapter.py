...
class RadioPipelineAdapter:
    ...
    def convert_segment_to_audiosegment(
        ...
        primary_lang = radio_segment.get("primary_lang_iso639_3") or radio_segment.get("primary_lang")
    ...
    def export_segments_for_asr(
        ...
        for segment in filtered:
            try:
                audio_seg = self.convert_segment_to_audiosegment(
                    segment,
                    shard_data,
                    source_data
                )
        ...
        for shard in shards:
            ...
            for segment in filtered:
                try:
                    ...
                    logger.warning(
                        "Failed to convert segment in batch",
                        error=str(e)
                    )
                    continue
