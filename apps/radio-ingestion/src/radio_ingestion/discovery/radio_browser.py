...
class RadioBrowserClient:
    ...
    def search_stations(
        ...
        """
        params = {"limit": limit, "offset": offset, "order": order, "reverse": str(reverse).lower()}
        ...
        try:
            logger.info("Searching Radio Browser API", url=url, params=params)
            ...
            while True:
                try:
                    ...
                    logger.info("Radio Browser API search successful", stations_found=len(stations))
            ...
            logger.error("Radio Browser API request failed", error=str(e), url=url, params=params)
            raise
    ...
    def get_station_by_uuid(self, station_uuid: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error("Failed to get station by UUID", station_uuid=station_uuid, error=str(e))
            return None
    ...
    def parse_station(self, station_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "country": (
                station_data.get("countrycode", "").upper()
                if station_data.get("countrycode")
                else None
            ),
...
def discover_stations(
    ...
    for station in stations:
        try:
            ...
            logger.warning(
                "Failed to parse station", station_name=station.get("name"), error=str(e)
            )
            continue
