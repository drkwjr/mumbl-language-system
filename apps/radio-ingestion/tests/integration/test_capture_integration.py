...
from radio_ingestion.storage.radio_repositories import RadioShardRepository, RadioSourceRepository
...
async def test_scheduler_with_mock_recorder(tmp_path):
    ...
    task = await scheduler.schedule_capture(
        source_id=1, stream_url="https://example.com/test", station_name="Test Station", duration=10
    )
