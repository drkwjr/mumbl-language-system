...
def test_convert_and_store_in_audio_segments(tmp_path):
    ...
    with get_connection() as conn:
        ...
        for seg in segments:
            try:
                audio_seg = adapter.convert_segment_to_audiosegment(seg, test_shard, test_source)

                # Store in audio_segments table
                segment_id = audio_repo.insert(
                    audio_seg, batch_id="radio_test", granularity="sentence", sample_rate=22050
                )
...
def test_batch_export_with_filters(tmp_path):
    ...
    with get_connection() as conn:
        exported = adapter.batch_export_for_asr(db_conn=conn, min_confidence=0.6, limit=10)
...
def test_filter_quality_segments():
    ...
    adapter = RadioPipelineAdapter(min_confidence=0.8, min_duration=2.0, max_duration=12.0)

    segments = [
        {"is_speech": True, "music_prob": 0.1, "confidence": 0.9, "duration": 5.0},
        {
            "is_speech": True,
            "music_prob": 0.1,
