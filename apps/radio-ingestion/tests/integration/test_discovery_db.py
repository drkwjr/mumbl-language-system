...
def test_discover_and_store_stations():
    ...
    with get_connection() as conn:
        ...
        assert any(sid is not None for sid in source_ids), "Should have at least one successful insert"
    ...
    with get_connection() as conn:
        ...
        if som_sources:
            for source in som_sources:
                ...
                assert country == "SOM", f"Source {source.get('id')} should be from Somalia, got {country}"
...
def test_update_last_check():
    ...
    with get_connection() as conn:
        ...
        assert updated["last_successful_capture"] is not None, "last_successful_capture should be set"
