...
class TestRadioPipelineAdapter:
    ...
    def test_init(self):
        ...
        adapter = RadioPipelineAdapter(min_speech_ratio=0.7, min_confidence=0.8)
    ...
    def test_convert_segment_to_audiosegment(self):
        ...
        shard_data = {"id": 1, "path": "/test/audio.wav", "s3_url": None, "source_id": 1}

        source_data = {"id": 1, "name": "Test Station", "country": "SO"}

        audio_seg = adapter.convert_segment_to_audiosegment(radio_segment, shard_data, source_data)
    ...
    def test_filter_high_quality_segments(self):
        ...
        adapter = RadioPipelineAdapter(
            min_speech_ratio=0.7, min_confidence=0.8, min_duration=2.0, max_duration=12.0
        )

        segments = [
            {"is_speech": True, "music_prob": 0.2, "confidence": 0.9, "duration": 5.0},
            {
            ...
            {"is_speech": True, "music_prob": 0.2, "confidence": 0.9, "duration": 1.0},  # Too short
            {"is_speech": True, "music_prob": 0.2, "confidence": 0.9, "duration": 15.0},  # Too long
        ]
    ...
    def test_export_segments_for_asr(self):
        ...
        with (
            patch(
                "radio_ingestion.integration.pipeline_adapter.RadioSegmentRepository"
            ) as mock_seg_repo_class,
            patch(
                "radio_ingestion.integration.pipeline_adapter.RadioShardRepository"
            ) as mock_shard_repo_class,
            patch(
                "radio_ingestion.integration.pipeline_adapter.RadioSourceRepository"
            ...
            mock_shard_repo.get_by_source.return_value = [
                {"id": 1, "source_id": 1, "path": "/test/audio.wav", "speech_ratio": 0.8}
            ]
