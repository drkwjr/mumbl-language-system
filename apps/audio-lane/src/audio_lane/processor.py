...
class AudioLaneProcessor:
    ...
    def process_youtube(
        self, url: str, language: str, batch_id: Optional[str] = None, dialect: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        download_result = download_audio(
            url, output_dir=os.path.join(self.output_base_dir, "raw"), language=language
        )
        raw_audio_path = download_result["audio_path"]
        duration = download_result["duration"]

        # Step 2: Normalize audio
        normalized_path = normalize_audio(raw_audio_path, sample_rate=self.sample_rate)
        ...
        csv_path = os.path.join(
            self.output_base_dir, f"paired_speech_corpus_{batch_id or 'default'}.csv"
        )
