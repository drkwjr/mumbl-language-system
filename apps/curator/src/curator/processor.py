...
class CuratorProcessor:
    ...
    def process_segments(
        ...
        text_scores_filtered = self._remove_duplicates_from_scores(
            text_scores, text_dups
        )
        audio_scores_filtered = self._remove_duplicates_from_scores(
            audio_scores, audio_dups
        )
        
        # Step 3: Apply policy gates
        eligible_text = self.policy_gate.get_eligible_segments(
            text_scores_filtered, target=target
        )
        ...
        return {
            ...
            'stats': {
                ...
                'exact_duplicates_removed': (
                    len(text_dups) + len(audio_dups)
                ),
                'near_duplicates_found': len(dedup_report.get('near_duplicates', {}).get('pairs', [])),
            },
