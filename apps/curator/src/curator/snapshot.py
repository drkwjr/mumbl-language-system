...
class DatasetSnapshot:
    ...
    def export_jsonl(self, snapshot: Dict[str, Any], segments_data: List[Dict[str, Any]]) -> str:
        """
        ...
        filtered_segments = [seg for seg in segments_data if seg.get("id") in segment_ids_set]
    ...
    def _get_next_version(self, language: str, dialect: str) -> str:
        ...
        existing_files = [
            f for f in os.listdir(self.output_dir) if f.startswith(prefix) and f.endswith(".json")
        ]
    ...
    def list_snapshots(
        self, language: Optional[str] = None, dialect: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
